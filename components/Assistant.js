...
const PROMPTS = {
  parse: (t) => `Проанализируй резюме и верни ТОЛЬКО JSON (без markdown, без текста вокруг). Заполни ВСЕ поля реальными данными из резюме. Формат: {"name":"имя","contacts":{"email":"email","phone":"телефон","city":"город"},"position":"должность","experience_years":5,"summary":"описание кандидата","skills":["навык1","навык2"],"experience":[{"company":"компания","role":"роль","period":"2020-2023","achievements":"достижения"}],"education":[{"institution":"вуз","degree":"специальность","year":"2015"}],"strengths":["сильная сторона"],"red_flags":["риск если есть"]}\n\nРезюме:\n${t}`,
  profile: (t) => `Составь профиль должности и верни ТОЛЬКО JSON (без markdown, без текста вокруг). Заполни ВСЕ поля конкретными данными. Формат: {"title":"должность","department":"отдел","purpose":"цель роли","responsibilities":["обязанность1","обязанность2","обязанность3"],"must_have":["требование1","требование2"],"nice_to_have":["желательно1"],"soft_skills":["навык1","навык2"],"kpi":["метрика1","метрика2"],"salary_range":"100000-150000 руб","growth_path":"карьерный путь"}\n\nОписание вакансии:\n${t}`,
  score: (t) => `Оцени кандидата на соответствие вакансии и верни ТОЛЬКО JSON (без markdown, без текста вокруг). Формат: {"overall_score":75,"verdict":"РАССМОТРЕТЬ","verdict_reason":"причина решения","match":[{"criteria":"критерий","score":7,"comment":"комментарий"}],"pros":["плюс1","плюс2"],"cons":["минус1"],"questions_to_clarify":["вопрос1","вопрос2"]}\n\nДанные:\n${t}`,
  interview: (t) => `Составь вопросы для интервью и верни ТОЛЬКО JSON (без markdown, без текста вокруг). Формат: {"competency":[{"competency":"компетенция","question":"вопрос","what_looking_for":"что оцениваем"}],"situational":["STAR вопрос 1","вопрос 2"],"technical":["тех вопрос 1","вопрос 2"],"motivation":["вопрос о мотивации"],"closing":["завершающий вопрос"]}\n\nДанные о вакансии:\n${t}`,
};
...
export default function Assistant({ onLogout }) {
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      if (!text) {
        alert('Пустой ответ от API. Попробуйте ещё раз.');
        setLoading(false);
        return;
      }
      ...
      if (!jsonMatch) {
        alert('Не удалось разобрать ответ. Попробуйте ещё раз.');
        setLoading(false);
        return;
      }
