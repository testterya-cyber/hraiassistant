...
const PROMPTS = {
  ...
  interview: (t) => `Ты интервьюер. Составь вопросы для интервью. Верни ТОЛЬКО валидный JSON без markdown:\n{"competency":[{"competency":"","question":"","what_looking_for":""}],"situational":[],"technical":[],"motivation":[],"closing":[]}\n${t}`,
};
  ...
  return (
    <div style={{ display: "grid", gap: "12px" }}>
      ...
      {d.questions_to_clarify?.length > 0 && (
        <div style={{ background: "#0d0d1a", border: "1px solid #1e1e3a", borderRadius: "10px", padding: "14px" }}>
          <div style={{ fontSize: "10px", color: "#555", textTransform: "uppercase", marginBottom: "8px" }}>Уточнить на интервью</div>
          {d.questions_to_clarify.map(q => <div key={q} style={{ fontSize: "13px", color: "#ccc", marginBottom: "5px" }}>? {q}</div>)}
        </div>
      )}
    </div>
  );
...
export default function Assistant({ onLogout }) {
  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif", background: "#07070f", minHeight: "100vh", color: "#e0e0e0", display: "flex", flexDirection: "column" }}>
      {/* Header */}
      <div style={{ borderBottom: "1px solid #1a1a2e", padding: "12px 22px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          <div style={{ width: "30px", height: "30px", borderRadius: "8px", background: "linear-gradient(135deg,#4ECDC4,#FF6B9D)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "14px" }}>🤖</div>
          <div>
            <div style={{ fontSize: "14px", fontWeight: "700", color: "#fff" }}>HR AI Ассистент</div>
            <div style={{ fontSize: "10px", color: "#333" }}>Powered by Claude</div>
          </div>
        </div>
        <button onClick={onLogout} style={{ background: "none", border: "1px solid #1e1e3a", borderRadius: "6px", color: "#444", fontSize: "12px", padding: "6px 14px", cursor: "pointer" }}>Выйти</button>
      </div>
